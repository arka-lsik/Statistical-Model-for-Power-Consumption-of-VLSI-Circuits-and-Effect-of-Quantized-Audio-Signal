...
This study explores how the noise came and average distortion can generate at detection end of practical end-to-end communication systems. The next part of this study explores where noise is not only introduced at the signal receiving end but also can accumulate at any intermediate stages, particularly at quantization, source encoding \& channel encoding etc. Now it reflects the effect of data compression required by transmission over bandwidth-limited channels. In this context, I modeled the big data using a Gaussian mixture model (GMM), which undergoes uniform and entropy-coded quantization before clustering through the expectation-maximization (EM) algorithm. The analysis explores how clustering accuracy is affected by the number of quantization levels. This effective unsupervised machine learning algorithm, on distribution-based clustering, can also address challenges in the VLSI design flow (It can be ASICs or FPGA), particularly in estimating CPU or GPU power and optimizing power consumption of VLSI circuits. The approach is modeled through statistical predictive analysis and evaluated using several performance metrics for comprehensive comparison. Overall, this study provides a framework that connects the solution of signal processing problems to VLSI circuit domain challenges through the use of mathematical modeling.

→  **Keyword** : GMM(Gaussian Mixture Model), Noise, Quantization, EM. Power, Estimation

🎯 **Problem Statement**

● $\textcolor{red}{Data \space Compression \space and \space Quantization \space in \space Bandwidth \space Limited \space Channels}$:
...
- Current research direction shifted toward a more impactful area: Power Analysis in VLSI Design flow.
- The main goal is to estimate and model microarchitectural or netlist level EDA data to support power-aware design.
...
- This data set was taken from $\textcolor{red}{ISCAS'89}$ conference, which contains 20 different benchmark sequential circuits.
...
- In the end column it gives the Mc(Monte Carlo) in mw simulated power. I have to estimate as close as possible.

As this data set is old and with small quantities of data samples, there may be some limitations to judge this model performance at the industry level. 
...
- This work data consider only the higher performance Cortex-A15 cluster.

🎯 **Model's Mathematical Descriptions**

The core idea is to model the joint distribution of input features $\textcolor{green}{(X)}$ and target variable $\textcolor{green}{(Y)}$ using a mixture of Gaussian distributions, and then use this joint model to predict $\textcolor{green}{Y \space given \space X}$. Let's break down the mathematics for the $\textcolor{green}{first \space case}$ of dataset- 
...
- $\textcolor{red}{Concept:}$ A GMM assumes that data points ($$z_n$$ which are $$[x_n, y_n]$$ in my case) are generated from a finite number of underlying Gaussian (Normal) distributions. These distributions are mixed together with certain probabilities.
...
where:
- K: The number of Gaussian components (mixtures).
...
- $\textcolor{red}{Purpose:}$ Given the observed data points $$XY_{norm}$$, the EM algorithm is used to estimate the unknown GMM parameters (θ) when we don't know which data point belongs to which Gaussian component. It's an iterative approach.
...
- $\textcolor{red}{E-step \space (Expectation \space Step):}$ assuming the current parameters are correct, calculate the "responsibility" (or posterior probability) that each component k has for generating each data point $$z_n$$ -
...
- $\textcolor{red}{Regularization \space Value:}$ This parameter adds a small positive constant to the diagonal of the covariance matrices during the M-step. Its purpose is to:
...
- Before discuss results I just wanted to revisit some aspect related to how I thought for this case of dataset. Mathematical concepts is almost same,
...
To know for $\textcolor{green}{mathemtical \space foundation \space of \space this \space part}$ of the project kindly visit either the PDF attached as *MTP* [page 12-23] or [Link](https://drive.google.com/drive/folders/1U_O50OhLvYyswf1Ef5FHRn-KYWHdfvlK?usp=drive_link).
